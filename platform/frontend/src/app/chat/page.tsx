...
export default function ChatPage() {
  ...
  useEffect(() => {
    console.log("[DEBUG page] useEffect for initialModel - current initialModel:", initialModel);
    ...
    if (!initialModel) {
      ...
      if (providers.length > 0) {
        ...
        console.log("[DEBUG page] firstProvider:", firstProvider, "models:", models);
        if (models && models.length > 0) {
          console.log("[DEBUG page] Setting initialModel to:", models[0].id);
          setInitialModel(models[0].id);
        }
      } else {
        console.log("[DEBUG page] No providers available, cannot set initialModel");
      }
    }
